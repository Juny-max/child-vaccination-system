...
export class BackupService {
   ...
   *
   * Expected layout: [ IV (12 bytes) | Auth Tag (16 bytes) | Ciphertext ]
   */
  decryptData(encryptedData: Buffer, key: string): Buffer {
    ...
    const authTag = encryptedData.slice(12, 28); // 16 bytes
    ...
    return Buffer.concat([
      ...
      decipher.final(), // throws if authentication tag verification fails
    ]);
  }
}
